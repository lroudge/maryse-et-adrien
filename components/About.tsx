  ...
  const googleEvent: GCalendarEventQuery = {
    ...
    const countdownRenderer = ({ days, hours, minutes, seconds, completed }: {days: number, hours: number, minutes: number, seconds: number, completed: boolean}) => {
      if (completed) {
        ...
        return <span className="italic p-5 mt-3 text-l md:text-2xl text-emerald-700">Dans <b>{days}</b> jours, <b>{hours}</b> heures, <b>{minutes}</b> minutes et <b>{seconds}</b> secondes...</span>;
      }
    }
